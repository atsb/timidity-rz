...
IT_Init (void)
{
  if (!(mh = (ITHEADER *) _mm_rz_doom_malloc (sizeof (ITHEADER))))
    return 0;
  if (!(poslookup = (UBYTE *) _mm_rz_doom_malloc (256 * sizeof (UBYTE))))
    return 0;
  if (!(itpat = (ITNOTE *) _mm_rz_doom_malloc (200 * 64 * sizeof (ITNOTE))))
    return 0;
  if (!(mask = (UBYTE *) _mm_rz_doom_malloc (64 * sizeof (UBYTE))))
    return 0;
  if (!(last = (ITNOTE *) _mm_rz_doom_malloc (64 * sizeof (ITNOTE))))
    return 0;
...
IT_Load (BOOL curious)
{
  ...
  if (!AllocPositions (mh->ordnum))
    return 0;
  if (!(origpositions = _mm_rz_doom_calloc (mh->ordnum, sizeof (UWORD))))
    return 0;
  ...
  if (!(paraptr = (ULONG *) _mm_rz_doom_malloc ((mh->insnum + mh->smpnum + of.numpat) *
					sizeof (ULONG))))
    return 0;
