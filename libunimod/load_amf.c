...
AMF_Init (void)
{
  if (!(mh = (AMFHEADER *)_mm_rz_doom_malloc(sizeof (AMFHEADER))))
    return 0;
  if (!(track = (AMFNOTE *)_mm_rz_doom_calloc(64, sizeof (AMFNOTE))))
    return 0;
...
AMF_Load (BOOL curious)
{
  ...
  /* read track table */
  if (!(track_remap = _mm_rz_doom_calloc (mh->numtracks + 1, sizeof (UWORD))))
    return 0;
