...
S69_Init (void)
{
  if (!(s69pat = (S69NOTE *)_mm_rz_doom_malloc(64 * 8 * sizeof (S69NOTE))))
    return 0;
  if (!(mh = (S69HEADER *)_mm_rz_doom_malloc(sizeof (S69HEADER))))
    return 0;
...
S69_Load (BOOL curious)
{
  ...
  if ((mh->message[0]) || (mh->message[36]) || (mh->message[72]))
    if ((of.comment = (CHAR *)_mm_rz_doom_malloc(3 * (36 + 1) + 1)))
      {
