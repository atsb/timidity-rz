...
MED_Init (void)
{
  if (!(me = (MEDEXP *) _mm_rz_doom_malloc (sizeof (MEDEXP))))
    return 0;
  if (!(mh = (MEDHEADER *) _mm_rz_doom_malloc (sizeof (MEDHEADER))))
    return 0;
  if (!(ms = (MEDSONG *) _mm_rz_doom_malloc (sizeof (MEDSONG))))
    return 0;
  return 1;
}
...
LoadMMD0Patterns (void)
{
  ...
  if (!(mmd0pat = (MMD0NOTE *) _mm_rz_doom_calloc (of.numchn * (maxlines + 1), sizeof (MMD0NOTE))))
    return 0;
...
LoadMMD1Patterns (void)
{
  ...
  if (!(mmd1pat = (MMD1NOTE *) _mm_rz_doom_calloc (of.numchn * (maxlines + 1), sizeof (MMD1NOTE))))
    return 0;
...
MED_Load (BOOL curious)
{
  ...
  if (!(ba = (ULONG *) _mm_rz_doom_calloc (ms->numblocks, sizeof (ULONG))))
    return 0;
  ...
  if ((mh->pMEDEXP) && (me->songname) && (me->songnamelen))
    {
      ...
      name = _mm_rz_doom_malloc (me->songnamelen);
