...
XM_Init (void)
{
  if (!(mh = (XMHEADER *) _mm_rz_doom_malloc (sizeof (XMHEADER))))
    return 0;
  return 1;
}
...
LoadPatterns (BOOL dummypat)
{
  ...
  for (t = 0; t < mh->numpat; t++)
	...
	_mm_fseek (modreader, ph.size, SEEK_CUR);
      ...
      if (ph.numrows)
	{
	  if (!(xmpat = (XMNOTE *) _mm_rz_doom_calloc (ph.numrows * of.numchn, sizeof (XMNOTE))))
	    return 0;
  ...
  if (dummypat)
    {
      of.pattrows[t] = 64;
      if (!(xmpat = (XMNOTE *) _mm_rz_doom_calloc (64 * of.numchn, sizeof (XMNOTE))))
	return 0;
      for (v = 0; v < of.numchn; v++)
