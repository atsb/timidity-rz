...
{
  if (!sl_buffer)
    if (!(sl_buffer = _mm_rz_doom_malloc (SLBUFSIZE * sizeof (SWORD))))
      return 0;
...
SL_Load (struct SAMPLOAD *sload)
{
  ...
  if (!(data = (SWORD *) _mm_rz_doom_malloc ((length + 20) << 1)))
    {
...
SL_RegisterSample (SAMPLE * s, URL reader)
{
  ...
  if (!(news = (SAMPLOAD *) _mm_rz_doom_malloc (sizeof (SAMPLOAD))))
    return NULL;
...
ML_InfoLoader (void)
{
  ...
  if (len)
    if ((list = _mm_rz_doom_malloc (len * sizeof (CHAR))))
      {
	list[0] = 0;
  ...
  if (len)
    {
      ...
      if (!(of.comment = (CHAR *) _mm_rz_doom_malloc (len + 1)))
	return 0;
...
ReadLinedComment (UWORD lines, UWORD linelen)
{
  ...
  if (lines)
    {
      if (!(tempcomment = (CHAR *) _mm_rz_doom_malloc (len + 1)))
	return 0;
      if (!(storage = (CHAR *) _mm_rz_doom_malloc (linelen + 1)))
	{
	  free (tempcomment);
	  return 0;
	}
      ...
      for (line = tempcomment, total = t = 0; t < lines; t++, line += linelen)
	{
      ...
      if (total > lines)
	{
	  if (!(of.comment = (CHAR *) _mm_rz_doom_malloc (total + 1)))
	    {
	      free (storage);
	      free (tempcomment);
	      return 0;
...
AllocPositions (int total)
{
  ...
  if (!(of.positions = _mm_rz_doom_calloc (total, sizeof (UWORD))))
    return 0;
  return 1;
}
...
AllocPatterns (void)
{
  ...
  if (!(of.patterns = (UWORD *) _mm_rz_doom_calloc ((ULONG) (of.numpat + 1) * of.numchn, sizeof (UWORD))))
    return 0;
  if (!(of.pattrows = (UWORD *) _mm_rz_doom_calloc (of.numpat + 1, sizeof (UWORD))))
    return 0;
...
AllocTracks (void)
{
  ...
  if (!(of.tracks = (UBYTE **) _mm_rz_doom_calloc (of.numtrk, sizeof (UBYTE *))))
    return 0;
  return 1;
}
...
AllocInstruments (void)
{
  ...
  if (!(of.instruments = (INSTRUMENT *) _mm_rz_doom_calloc (of.numins, sizeof (INSTRUMENT))))
    return 0;
...
AllocSamples (void)
{
  ...
  if (!(of.samples = (SAMPLE *) _mm_rz_doom_calloc (of.numsmp, sizeof (SAMPLE))))
    return 0;
...
DupStr (CHAR * s, UWORD len, BOOL strict)
{
  ...
  if ((d = (CHAR *) _mm_rz_doom_malloc (sizeof (CHAR) * (len + 1))))
    {
      for (t = 0; t < len; t++)
	d[t] = (s[t] < 32) ? '.' : s[t];
      d[len] = 0;
...
ML_AllocUniMod (void)
{
  return (_mm_rz_doom_malloc (sizeof (MODULE)));
}
