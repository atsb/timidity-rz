...
FAR_Init (void)
{
  if (!(mh1 = (FARHEADER1 *)_mm_rz_doom_malloc(sizeof (FARHEADER1))))
    return 0;
  if (!(mh2 = (FARHEADER2 *)_mm_rz_doom_malloc(sizeof (FARHEADER2))))
    return 0;
  if (!(pat = (FARNOTE *)_mm_rz_doom_malloc(256 * 16 * 4 * sizeof (FARNOTE))))
    return 0;
