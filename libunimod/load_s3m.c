...
S3M_Init (void)
{
  if (!(s3mbuf = (S3MNOTE *) _mm_rz_doom_malloc (32 * 64 * sizeof (S3MNOTE))))
    return 0;
  if (!(mh = (S3MHEADER *) _mm_rz_doom_malloc (sizeof (S3MHEADER))))
    return 0;
  if (!(poslookup = (UBYTE *) _mm_rz_doom_malloc (sizeof (UBYTE) * 256)))
    return 0;
...
S3M_Load (BOOL curious)
{
  ...
  if (!(paraptr = (UWORD *) _mm_rz_doom_malloc ((of.numins + of.numpat) * sizeof (UWORD))))
    return 0;
